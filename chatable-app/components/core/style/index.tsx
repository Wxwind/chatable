...
// useTheme hook
export function useTheme<T>(props: { themeStyles: (theme: Theme) => T; styles?: Partial<T> }): T {
...
export function WithTheme<T, S>(props: WithThemeProps<T, S>) {
  ...
  return <ThemeContext.Consumer>{(theme) => children(getStyles(theme), theme)}</ThemeContext.Consumer>;
}
