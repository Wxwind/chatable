...
import HapticsContext, { HapticsContextProvider, HapticsFunc } from './HapticsContext';
...
const Provider: React.FC<ProviderProps> = (props) => {
  const { children, theme, onHaptics, isRTL } = props;
  ...
  if (!hapticsContext && typeof onHaptics === 'function' && Platform.OS !== 'web') {
    childNode = <HapticsContextProvider onHaptics={onHaptics}>{childNode}</HapticsContextProvider>;
  }

  if (typeof isRTL === 'boolean') {
    childNode = <AccessibilityContextProvider isRTL={isRTL}>{childNode}</AccessibilityContextProvider>;
  }

  return <ThemeProvider value={theme}>{childNode}</ThemeProvider>;
};
