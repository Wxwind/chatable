...
import { ActivityIndicator, Animated, Text, View, ViewStyle } from 'react-native';
...
const ToastContainer: FC<ToastProps> = (p) => {
  ...
  const { icon, type = '', mask, duration, content, onAnimationEnd, onClose, position } = props;
  ...
  const iconDom = useMemo(() => {
    ...
    switch (typeTemp) {
      case 'loading':
        return <ActivityIndicator animating style={[styles.centering]} color="white" size="large" />;
      case 'info':
        return icon;
      default:
        const iconType: {
        ...
        return <Icon name={iconType[type]} style={styles.image} color="white" size={36} />;
    }
  ...
  return (
    <View style={[positionStyle, styles.container]} pointerEvents={mask ? undefined : 'box-none'}>
      <View style={[styles.innerContainer]}>
        <Animated.View style={{ opacity: fadeAnim.current }}>
          <View style={[styles.innerWrap, iconDom ? styles.iconToast : styles.textToast]}>
            {iconDom}
            {React.isValidElement(content) ? content : <Text style={styles.content}>{content}</Text>}
          </View>
        </Animated.View>
      </View>
    </View>
