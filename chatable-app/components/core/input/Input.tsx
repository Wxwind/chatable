...
const InternalInput: React.ForwardRefRenderFunction<TextInput, InputProps> = (props, ref) => {
  const {
    allowClear,
    disabled,
    readOnly,
  ...
  // ========================= Status =========================
  const [innerStatus, setInnerStatus] = useMergedState<InputStatus | undefined>(undefined, {
    value: customStatus,
  });
  useEffect(() => {
  ...
  }, [innerValue?.length, maxLengthConfig.show, maxLengthConfig.value, setInnerStatus]);

  // getStatusClassNames
  const statusClassName = useMemo(() => {
    const mergedStatus = innerStatus;
    ...
    return undefined;
  }, [innerStatus, styles.error, styles.warning]);
  ...
  const restProps = React.useMemo(() => {
    ...
    if (rest.value === undefined && (allowClear || maxLengthConfig.show || showCount || type === 'number')) {
  ...
  const prefixDom = useMemo(() => {
    if (prefix) {
      return <AutoView style={[styles.prefix, statusClassName]}>{prefix}</AutoView>;
    }
  ...
  const clearIconDom = useMemo(() => {
    if (!disabled && editable && allowClear && innerValue && focus) {
      return (
        ...
        >
          {(allowClear as ClearIconType).clearIcon || <Icon name="close" color={'white'} size={13} />}
        </TouchableOpacity>
      );
    }
  }, [allowClear, disabled, editable, focus, innerValue, pushInputValue, styles.clearIcon]);

  const showCountDom = useMemo(() => {
    const length = innerValue?.length || 0;
    let node;
    if (typeof showCount === 'object' && typeof showCount.formatter === 'function') {
  ...
  }, [innerValue, maxLengthConfig.show, maxLengthConfig.value, showCount, statusClassName, styles.showCount]);

  const suffixDom = useMemo(() => {
    if (suffix) {
      return <>{Boolean(suffix) && <AutoView style={[styles.suffix, statusClassName]}>{suffix}</AutoView>}</>;
    }
  }, [statusClassName, styles.suffix, suffix]);
