  ...
  const restTextAreaProps: TextAreaProps = useMemo(() => {
    ...
    return {
      ...
      style: [
        {
          // `autoSize={{  maxRows: 5 }}`
          maxHeight: maxRows > 0 && maxRows > minRows ? firstLayoutHeight : undefined,
        },
        props.style,
      ],
    };
  }, [autoSize, firstLayoutHeight, maxRows, minRows, numberOfLines, onLayout, props, rows, showPlaceholder]);

  return <Input themeStyles={TextAreaStyle} {...restProps} multiline={true} {...restTextAreaProps} ref={ref} />;
});
